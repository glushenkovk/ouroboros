...
def _tool_read_file(args: dict, memory: Memory) -> str:
    path = args["path"]
    if not os.path.exists(path):
...
def _tool_write_file(args: dict, memory: Memory) -> str:
    path = args["path"]
...
def _tool_list_dir(args: dict, memory: Memory) -> str:
    path = args.get("path", ".")
    if not os.path.exists(path):
...
def _tool_run_shell(args: dict, memory: Memory) -> str:
...
def _tool_web_search(args: dict, memory: Memory) -> str:
    import httpx
    import re
    query = args["query"]
...
def _tool_http_get(args: dict, memory: Memory) -> str:
    import httpx
    resp = httpx.get(args["url"], timeout=15, follow_redirects=True)
    return resp.text[:5000]


def _tool_http_post(args: dict, memory: Memory) -> str:
    import httpx
    resp = httpx.post(args["url"], json=args["body"], timeout=15)
    return resp.text[:3000]


def _tool_read_scratchpad(args: dict, memory: Memory) -> str:
    return memory.read_scratchpad() or "(empty)"


def _tool_update_scratchpad(args: dict, memory: Memory) -> str:
    memory.write_scratchpad(args["content"])
    return "Scratchpad updated"


def _tool_read_identity(args: dict, memory: Memory) -> str:
    return memory.read_identity() or "(empty)"


def _tool_update_identity(args: dict, memory: Memory) -> str:
    memory.write_identity(args["content"])
    return "Identity updated"


def _tool_task_board(args: dict, memory: Memory) -> str:
    action = args["action"]
    with _tasks_lock:
        tasks = _load_tasks()
...
def _tool_generate_image(args: dict, memory: Memory) -> str:
    import httpx
    prompt = args["prompt"]
...
def _tool_send_message(args: dict, memory: Memory) -> str:
    text = args.get("text", "")
...
_HANDLERS = {
    ...
    "update_scratchpad": _tool_update_scratchpad,
    "read_identity": _tool_read_identity,
    "update_identity": _tool_update_identity,
    "task_board": _tool_task_board,
